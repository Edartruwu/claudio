...
// dynamic (per-session) content in the system prompt. Everything before this
// marker can be cached globally; everything after is session-specific.
...
func introSection() string {
	return `You are Claudio, an AI-powered coding assistant for the terminal.
...
func sessionGuidanceSection() string {
 ...
 - If you do not understand why the user has denied a tool call, ask them.
...
func environmentSection(model string) string {
	...
	knowledgeCutoff := knowledgeCutoffDate(model)
