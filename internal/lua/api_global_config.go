...
func (r *Runtime) injectGlobalConfigAPI(L *lua.LState, claudio *lua.LTable) {
	// claudio.config sub-table
	cfgTable := L.NewTable()
	L.SetField(cfgTable, "set", L.NewFunction(r.apiGlobalConfigSet()))
	L.SetField(cfgTable, "get", L.NewFunction(r.apiGlobalConfigGet()))
	L.SetField(claudio, "config", cfgTable)

	// claudio.ui sub-table — stubs; real implementations supplied by lua-ui-apis
	// when that module is loaded. These no-ops ensure defaults.lua never errors.
	uiTable := L.NewTable()
